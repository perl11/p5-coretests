...
foreach my $optimization (@optimizations) {
  TODO: SKIP: {
    ...
    unless ( $errors->check_todo( $signal == 0, 
                                  "Exit signal is $signal $sig_name", 'SIG' ) ) {
      note $out if $out;
      skip( "Test failures irrelevant if exits premature with $sig_name", $errors->{to_skip} );
    }

    unless ( $errors->check_todo( $parser->{is_good_plan}, 
                                  "Plan was valid", 'PLAN' ) ) {
      note $out if $out;
      skip( "TAP parse is unpredictable when plan is invalid", $errors->{to_skip} );
    }

    my $tests_ok = $errors->check_todo( $parser->{exit} == 0, 
                                        "Exit code is $parser->{exit}", "EXIT" );
    $tests_ok = $errors->check_todo( !scalar @{ $parser->{failed} }, 
                                     "Test results:", 'TESTS' ) && $tests_ok;
    print "    $_\n" foreach ( split( "\n", $out ) );
