...
sub cmp_ok ($$$@) {
    my ( $Xgot, $Xtype, $Xexpected, $Xname, @Xmess ) = @_;
    ...
    my $Xpass;
    {
        ...
        $Xpass = eval "\$Xgot $Xtype \$Xexpected";
    }
    unless ($Xpass) {
        ...
        if ( $Xgot eq $Xexpected and $Xtype !~ tr/a-z// ) {
            unshift @Xmess, "# $Xgot - $Xexpected = " . ( $Xgot - $Xexpected ) . "\n";
        }
        unshift(
            @Xmess, "#      got " . _qq($Xgot) . "\n",
            "# expected $Xtype " . _qq($Xexpected) . "\n"
        );
    }
    _ok( $Xpass, _where(), $Xname, @Xmess );
}
...
sub eq_hash {
    ...
    while ( my ( $key, $value ) = each %$suspect ) {
        ...
        $key = "" . $key;
        if ( exists $orig->{$key} ) {
            if ( defined $orig->{$key} != defined $value
    ...
    if ( !$threads_on || $method eq "process" ) {
        ...
        if ( $is_mswin || $is_vms ) {
            ...
            eval {
                ...
                my $prog = "sleep($timeout);" . "warn qq/# $timeout_msg" . '\n/;' . "kill(q/$sig/, $pid_to_kill);";
